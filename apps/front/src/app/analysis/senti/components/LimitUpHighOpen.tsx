...
const LimitUpHighOpen = ({ dateRange }: IProps) => {
  const [loading, setLoading] = useState<boolean>(false);
  const [sourceData, setSourceData] = useState<NSGetAnalysisSentiList.IRes>([]);
  const { requestConfig, runLatestRequest } = useLatestRequest('senti-limit-up-high-open');
   ...
   */
  const genOptions = () => {
    const min = getRoundedMin(sourceData.map((item) => +item.sentiB));
    const max = getRoundedMax(sourceData.map((item) => +item.sentiB));
...
export default LimitUpHighOpen;
