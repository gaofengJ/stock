...
const RateLimitUp1to2 = ({ dateRange }: IProps) => {
  ...
  const { requestConfig, runLatestRequest } = useLatestRequest('chains-rate-limit-up-1-to-2');
   ...
   */
  const genOptions = () => {
    const min = getRoundedMin(sourceData.map((item) => item.rate));
    const max = getRoundedMax(sourceData.map((item) => item.rate));
